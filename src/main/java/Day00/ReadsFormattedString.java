...
public interface ReadsFormattedString {
  static <T> T readString(String s, String pattern, Class<T> target) {
    ...
    //  checkState(target.getConstructors().length > 0, "Class "+target+" has no constructor!");
      return (T) Arrays.stream(target.getConstructors()).filter(c -> c.getParameterCount() == mappedObjs.size()).findAny().get()
                                   //.getConstructor(mappedObjs.stream().map(Object::getClass).toArray(Class[]::new))
  ...
  static int crunchNumber(String s, String pattern) {
    return Integer.parseInt(crunchString(s, pattern));
  }
